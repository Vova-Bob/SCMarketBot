...
def create_market_embed(listing: dict):
    ...
    embed.add_field(name=t("listings.field.item_type"), value=listing['item_type'].capitalize())
    if listing["listing_type"] != "unique":
        embed.add_field(name=t("listings.field.min_price"), value=f"{int(listing['minimum_price']):,} aUEC")
        embed.add_field(name=t("listings.field.max_price"), value=f"{int(listing['maximum_price']):,} aUEC")
    else:
        embed.add_field(name=t("listings.field.price"), value=f"{int(listing['price']):,} aUEC")
        embed.add_field(
            name=t("listings.field.seller"),
    ...
    if listing['auction_end_time'] is not None:
        ...
        embed.add_field(name=t("listings.field.auction_end"), value=t("listings.field.ending") + humanize.naturaltime(date))

    embed.add_field(name=t("listings.field.quantity_available"), value=f"{int(listing['quantity_available']):,}")
...
def create_market_embed_individual(listing: dict):
    ...
    embed.add_field(name=t("listings.field.item_type"), value=listing['details']['item_type'].capitalize())
    embed.add_field(name=t("listings.field.price"), value=f"{int(listing['listing']['price']):,} aUEC")
    ...
    embed.add_field(name=t("listings.field.quantity_available"), value=f"{int(listing['listing']['quantity_available']):,}")
...
async def display_listings_compact(interaction: discord.Interaction, alllistings: list):
    ...
    for listings in chunks(alllistings, 10):
        ...
        header = f"\u001b[4;40;37m {t('listings.stock.header.qt'):<{mq}} | {t('listings.stock.header.item'):<{tq}} | {t('listings.stock.header.price'):>{pq + 5}} "
